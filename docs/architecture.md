...
1. 采集主显示器画面，在内存中直接缩放到 768 × 432，不再经过“PNG 编码 → PNG 解码”往返；
...
5. 发送收敛后的识别提示（首句给最重要且明确的状态、第二句补 1–2 项相关信息、禁止虚构 /
   禁止操作建议）与图片，`max_tokens=160`，低温度 `0.1`，`cache_prompt=true`；
...
Rust 在启动识别时创建标签为 `floating` 的独立 Webview 窗口。它无系统边框、透明、置顶、不显示在任务栏，和主窗口使用同一前端入口；前端根据窗口标签渲染为原版精灵形象与消息气泡。气泡的右边界固定在精灵专属安全区之前，且原生窗口尺寸按“气泡 + 精灵安全区”计算，因此消息增长不会覆盖精灵；窗口尺寸调整在 WebView 侧做 180ms 防抖，避免每个模型 delta 都触发原生 resize。窗口可拖动；停止识别或点击关闭时隐藏。
...
本地视觉模型通过 OpenAI 兼容的 `POST /v1/chat/completions` 接口调用。模型提示明确收敛为“
首句给最重要且明确的当前状态、第二句补充 1–2 项直接相关信息”，并强制：单张截图不得声称时间
上的“变化”、模糊内容如实说明而非补全、不产生操作建议、步骤、坐标或动作协议。`max_tokens=160`
