...
1. 采集主显示器画面（Windows Graphics Capture）。窗口保持可见，不在截图瞬间
   cloak / 隐藏精灵与气泡。启动识别前先静默采一帧不含本应用窗口的桌面底板；
   之后每帧用底板盖住主窗口与悬浮窗所占矩形，识别画面穿透精灵 / 气泡 / 启动台。
   不用 `WDA_EXCLUDEFROMCAPTURE`：该标志会让 WebView2 无法绘制。画面在内存中
