...
1. 点击启动后，Rust 先静默采一帧不含本应用窗口的桌面底板，再以默认关注点
   开始识别循环并显示系统级悬浮精灵窗；
...
1. 采集主显示器画面（Windows Graphics Capture）。启动时由 Rust 宿主先静默采一帧
   不含本应用窗口的桌面底板，完成后才显示精灵与气泡，避免前端显示命令与底板采集竞态；
   之后每帧用底板盖住主窗口与悬浮窗所占矩形，识别画面穿透精灵 / 气泡 / 启动台。
   底板初始化、窗口句柄或窗口范围读取失败时绝不退回原始截图：改为逐帧临时 cloak，
   若仍无法可靠排除本应用窗口则跳过模型请求并报告错误。画面在内存中
   直接缩放到 768 × 432，不再经过“PNG 编码 → PNG 解码”往返；
...
Rust 在启动识别时创建标签为 `floating` 的独立 Webview 窗口。它无系统边框、透明、置顶、不显示在任务栏，和主窗口使用同一前端入口；前端根据窗口标签渲染为原版精灵形象与消息气泡。原生 HWND 跟随精灵与气泡的实测占位实时伸缩（精灵单独约 104×104，有识别结果时向左/上扩展），避免透明空白挡住下层点击。屏幕采集走 Windows Graphics Capture；识别循环不 cloak、不隐藏悬浮窗，只用启动前缓存的桌面底板盖住本应用窗口矩形，因此用户始终能看到完整精灵 / 气泡，识别画面也不包含它们。不用 `contentProtected` / `WDA_EXCLUDEFROMCAPTURE`，否则 WebView2 无法绘制。等待态只显示精灵、不弹出气泡；窗口本身不出现外部滚动条，过长文本在气泡内无滚动条滚动。窗口可拖动；停止识别时自动隐藏，点击关闭则先停止识别再隐藏。
