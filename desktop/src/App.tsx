...
const initialRuntime: RuntimeSnapshot = {
  protocolVersion: "1.0.0", mode: "mock · dry-run", phase: "idle", connected: true,
  inferenceBackend: "llama.cpp SYCL / mock", device: "SYCL0 · Intel Arc", modelReady: true,
  taskId: null, goal: null, message: "本地运行时已就绪",
};
      ...
      <div className="workspace">
        ...
        <section className="main-pane">
          <div className="content-scroll">
            ...
            <div className="mode-row"><button className={`mode-toggle ${live ? "selected" : ""}`} onClick={() => setLive(!live)}><span className="toggle"><i /></span><span><strong>{live ? "真实桌面模式" : "预览模式"}</strong><small>{live ? "仍需逐步确认，真实输入受安全策略保护" : "mock + dry-run，不会注入键鼠事件"}</small></span></button><div className="safe-badge"><ShieldCheck size={15} /> 高风险操作自动拦截</div></div>
