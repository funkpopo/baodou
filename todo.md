...
- [x] 定义任务状态：`idle` → `observing` → `planning` → `awaiting_confirmation` → `executing` → `verifying` → `completed/failed/paused`。
- [x] 支持用户自然语言任务，例如“打开浏览器并搜索……”“点击设置”“把这段文字输入到当前输入框”。
- [x] 将任务拆分为短步骤，每一步都绑定目标元素、前置条件、预期变化、风险等级和超时。
- [x] 优先使用 `element_id` 执行动作；只有在元素识别不可用时，才允许使用经过二次确认的坐标。
- [x] 定义基础动作：鼠标移动、单击、双击、右键、拖拽、滚轮、键盘按键、文本输入、快捷键、等待和重新识别。
- [x] 实现动作前重新截图/重新定位，防止页面变化后的旧坐标误操作。
- [x] 实现动作后验证：检查目标元素状态、页面文本变化、窗口变化或截图差异。
- [x] 支持失败恢复：重新识别、回到上一步、跳过可选步骤、暂停并请求用户处理。
- [x] 增加用户可见的“即将执行什么、目标在哪里、预计影响是什么”的预览。

**阶段验收：** 能完成至少三个低风险端到端任务，并且每一步都有可追踪的目标、执行结果和验证结果；页面变化或目标消失时会暂停，而不是盲目点击原坐标。

**阶段 F 完成记录（2026-08-11）：**
- 状态机：`agent/state.py` + `TaskAgent`（`agent/runtime.py`）
- 预览 / 恢复：`agent/preview.py` · `agent/recovery.py`
- 执行：`actuator/relocate.py`（element_id 优先）· `verify.py` · `win_input.py`（SendInput，默认 dry_run）
- CLI：`python -m frontend.cli agent run|preview|states`
- Bench：`python benchmarks/phase_f/run_agent_bench.py` → 低风险 4/4 + 目标消失暂停
- 文档：`docs/agent.md`；默认 `actuator.dry_run: true`

## 阶段 G：安全、权限与隐私

- [x] 建立风险分级：只读观察、低风险交互、中风险数据修改、高风险外部提交/不可逆操作。
- [x] 默认只读；低风险操作可由用户配置自动执行；中高风险操作必须逐步或最终确认。
- [x] 为敏感动作建立硬编码拦截和确认策略，包括删除、支付、转账、发送、发布、安装、权限变更和密码输入。
- [x] 支持全局暂停热键、紧急停止按钮和失去焦点自动暂停。
- [x] 限制动作频率、连续动作数量、单次任务最大时长和最大鼠标移动范围。
- [x] 对目标应用、窗口标题、域名和敏感区域设置允许/禁止列表。
- [x] 屏幕图像、OCR 文本、模型上下文和操作日志默认仅本地保存，并提供关闭持久化和清理功能。
- [x] 密码框、银行卡号、身份证号等内容默认遮罩，不发送给模型、不写入日志。
- [x] 记录不可抵赖的本地审计信息：用户任务、模型版本、识别结果摘要、确认记录、动作和结果。
- [x] 完成威胁建模，覆盖 prompt injection、恶意网页指令、模型幻觉、坐标欺骗、剪贴板泄漏和越权操作。

**阶段验收：** 通过安全测试集中的高风险场景；即使模型输出“立即执行”，也不能绕过权限、确认和动作白名单。

**阶段 G 完成记录（2026-08-11）：**
- 模块：`safety/{policy,risk,targets,limits,control,redact,audit,threats}`
- 默认：`safety.default_mode: read_only` · `block_high_risk: true` · 审计 `logs/audit/`
- 控制：`python -m frontend.cli safety status|pause|resume|stop|reset|check|redact|threats|audit`
- Bench：`python benchmarks/phase_g/run_safety_bench.py` → 高风险拦截 + 注入/坐标门控
- 文档：`docs/safety.md`；agent 每步集成 control + policy + limits + audit
